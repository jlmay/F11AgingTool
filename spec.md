...
| ⑤ | 扫码测试 | ❌ | 有扫码数据且与样本二维码一致 | 编辑框内容比对；每轮清空后等待扫码（10秒无数据超时），对比数据判定 |

### 扫码测试流程

**阶段一：样本设置**
1. 勾选"扫码测试"→ 可见 EditText 出现，自动获取焦点（`showSoftInputOnFocus=false` + `inputType="none"` 抑制软键盘）
2. 用扫码枪扫描样本二维码 → 数据通过 HID 键盘事件输入 EditText
3. 样本码接收结束条件（任一满足）：
   - 扫码枪发送回车（Enter）→ 弹窗确认
   - 最后数据接收后 300ms 无新数据 → 弹窗确认
4. 弹窗显示扫码内容，用户确认后 → 编辑框设为只读，样本码锁定
5. 用户取消 → 编辑框清空，可重新扫描；**若测试正在运行，取消将终止整个测试**（样本未确认则无法继续）
6. **未配置且未确认的样本码**（`barcodeSampleConfirmed == false`）→ 扫码测试项直接失败并终止测试

**阶段二：测试执行**
1. 每轮测试中，清空编辑框、设为可编辑、获取焦点（无软键盘）
2. 轮询等待扫码数据（**10 秒无数据超时**），每 50ms 检查一次
3. 用"文本稳定法"确保完整接收（连续 50ms 无新字符变化认为接收完毕）
4. 检测到数据后与阶段一确认的样本码对比
5. 一致 → 测试成功；不一致 → 测试失败
6. **恢复测试时（模式 2 重启后）**：样本码值及确认标志从 `SharedPreferences` 自动恢复，**不弹出**样本码确认对话框，直接进入扫码等待
7. **未配置样本码**（阶段一未确认样本，且持久化数据异常）→ 无条件终止测试，清除自动恢复标志，**不执行重启操作**（防止模式 2 无限重启）
...
- **主内容区**（从上到下，可滚动）：
  ...
  - 遇错停止（CheckBox，勾选后任一测试项失败即终止后续测试）
  - 扫码样本输入区（勾选扫码测试时显示）
...
- `====== 测试已手动停止 ======` — 手动停止标记
- `====== 全部测试完成 ======` — 会话结束标记
- `====== 因无样本二维码终止测试 ======` — 无样本码时终止标记
- `成功: X  失败: Y  总计: Z` — 最终统计
- 模式 2 重启失败/降级相关信息

**不记录的内容：** 测试成功时的详细调试信息（序列号、ATR、随机数等）、轮次正常进行的实时日志、**开机恢复消息**（仅屏幕显示，不写文件）。
...
| `auto_stop_on_error` | Boolean | 勾选遇错停止 |
| `auto_barcode_ref` | String | 样本二维码值 |
| `auto_barcode_confirmed` | Boolean | 样本二维码是否已确认 |
